...
class PetScorer:
    ...
    def calc_feature_score(self, trait: dict) -> float:
        ...
        # 14. 状态效果施加（基于实际效果强度）
        # 萌化>冻结(下场不清除)>寄生>中毒>灼烧
        status_pats = [
            (r"萌化", 18),      # 最强debuff：种族值降低60%-80%，下场不清除，最低形态无法继续萌化
            (r"冻结", 15),      # 强debuff：HP上限削减5%/层，下场不清除，冰系免疫
            (r"寄生", 12),      # 中等debuff：6%吸血/回合，下场清除，草系免疫
            (r"中毒", 9),       # 持续伤害：3%毒伤/回合，受克制影响，下场清除，毒系免疫
            (r"灼烧", 7),       # 持续伤害：2%火伤/回合，层数衰减，下场清除，火系免疫
            (r"麻痹", 6), (r"束缚", 5),
        ]
        ...
        # 15. 天气/场地效果（持续8回合，对双方生效，可顶替）
        weather_pats = [
            (r"暴风雪", 15),   # 强天气：每回合2层冰冻，冰系精灵强化
            (r"雨天", 12),      # 水系天气：水系技能威力+50%
            (r"沙暴", 10),      # 地系天气：地系技能能耗减半
        ]
        for pat, val in weather_pats:
            if re.search(pat, full_text):
                score += val
                break

        # 16. 战斗机制关键词（按战术价值）
        # 首领血脉(Mega)>愿力冲击(Z招式)>先手权>连击>迅捷>奉献>应对>脱离
        mech2_pats = [
            (r"首领血脉", 40),  # Mega进化级：首领形态，属性大幅提升，持续整场
            (r"愿力冲击", 28),  # Z招式级：高威力一次性大招，每场可用2次
            (r"先手\+", 25),    # 先手权：决定出手顺序，核心战术价值
            (r"连击", 20),      # 多次攻击：连击数×威力，爆发能力强
            (r"迅捷", 18),      # 换场触发：主动换人时自动放技能
            (r"奉献", 15),      # 全队共享强化：啃咬/虫群永久增强
            (r"应对", 10),      # 反制机制：预判敌方技能类型触发
            (r"脱离", 8), (r"紧急脱离", 8), (r"返场", 8),  # 离场切换
            (r"蓄力", 6),  # 延迟爆发机制
        ]
        for pat, val in mech2_pats:
            if re.search(pat, full_text):
                score += val
                break
    ...
    def calc_skill_score(self, skills: dict, attrs: list[str] = None) -> dict:
        ...
        for s in status_skills:
            ...
            if "翻倍增益" in desc:
                status_score += 6
            # 控制/状态效果（与特性评分价值一致）
            # 按效果强度加权：萌化>冻结>寄生>中毒>灼烧>其他
            for kw, val in [("萌化", 8), ("冻结", 6), ("寄生", 5),
                            ("中毒", 4), ("灼烧", 3), ("眩晕", 3),
                            ("麻痹", 3), ("冰冻", 3), ("恐惧", 2), ("迷惑", 2), ("束缚", 2)]:
                if kw in desc:
                    status_score += val
                    break

            # 天气/场地效果（持续8回合，对双方生效）
            for kw, val in [("暴风雪", 7), ("雨天", 6), ("沙暴", 5)]:
                if kw in desc:
                    status_score += val
                    break

            # 战斗机制关键词
            for kw, val in [("首领血脉", 15), ("愿力冲击", 12),
                            ("先手+", 10), ("连击", 9), ("迅捷", 8), ("奉献", 7),
                            ("应对", 5), ("脱离", 4), ("紧急脱离", 4), ("返场", 4), ("蓄力", 3)]:
                if kw in desc:
                    status_score += val
                    break
            # 迅捷/先手
    ...
    def recommend_skills(self, pet_id: str, top_n: int = 4) -> list[dict]:
        ...
        # 控制类关键词加分（与特性评分保持一致的价值评估）
        control_kw = {
            "首领血脉": 45,  # Mega进化级：首领形态，属性大幅提升，持续整场
            "愿力冲击": 32,  # Z招式级：高威力一次性大招
            "萌化": 25,      # 最强debuff：种族值降低60%-80%，下场不清除
            "先手+": 25,     # 先手权：决定出手顺序
            "冻结": 20,      # 强debuff：HP上限削减，下场不清除
            "连击": 22,      # 多次攻击：连击数×威力
            "迅捷": 20,      # 换场触发技能
            "奉献": 18,      # 全队共享强化
            "暴风雪": 18,   # 强天气：每回合2层冰冻
            "寄生": 16,      # 持续吸血debuff
            "雨天": 15,      # 水系天气：水系技能威力+50%
            "中毒": 12,      # 持续毒伤
            "沙暴": 12,      # 地系天气：地系技能能耗减半
            "应对": 11,      # 反制机制
            "灼烧": 10,      # 持续火伤（层数衰减）
